...
//   // Use highly accurate MLKit probability first, fallback to Mesh EAR if undefined
...
//   let zIQR=0;
//   if(mesh){const zv:number[]=[];for(const k of Object.keys(mesh)){const pts=mesh[k];for(let i=0;i<pts.length;i+=2)if(pts[i]?.z!==undefined)zv.push(pts[i].z!);}
...
//   return{detected:true,tooSmall:fa<0.12,tooBig:fa>0.65,yaw,pitch,roll,leftEAR,rightEAR,
//     leftIrisRatio,rightIrisRatio,smileScore,mouthOpen,noseTipY,zIQR,
//     flowPoints:[fp1,fp2,fp3,fp4,fp5] as [number,number][]};
// }

// // ALGORITHM THRESHOLDS & CONSTANTS
...
// // ========================================================
// // HOISTED COMPONENTS (Fixes "Cannot find name" errors)
// // ========================================================

// const GridBackground = React.memo(() => {
//   return (
//     <View style={StyleSheet.absoluteFill} pointerEvents="none">
//       {/* NO SOLID BACKGROUND HERE - Allowing Camera to show through */}
//       <View style={{position: 'absolute', width: '100%', height: '100%', opacity: 0.15}}>
//         {[...Array(20)].map((_, i) => (
//           <View key={`h-${i}`} style={{position: 'absolute', top: i * (SH/20), width: '100%', height: 1, backgroundColor: C.cyan}} />
//         ))}
//         {[...Array(10)].map((_, i) => (
//           <View key={`v-${i}`} style={{position: 'absolute', left: i * (SW/10), width: 1, height: '100%', backgroundColor: C.cyan}} />
//         ))}
//       </View>
//       <View style={{...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,5,12,0.45)'}} />
//     </View>
//   );
// });
...
//       let str = '';
//       for(let i=0;i<4;i++) str += Math.random().toString(16).substring(2,6).toUpperCase() + ' ';
//       setData(str);
//     }, 150);
//     return () => clearInterval(int);
//   }, []);
//   return <Text style={tc.matrix}>{data}</Text>;
// });
...
//   useEffect(()=>{ Animated.timing(glowAnim,{toValue:passed?1:0,duration:300,useNativeDriver:false}).start(); },[passed]);
...
//   return (
//     <View style={[hc.chip,{borderColor:passed?color+'88':color+'33',backgroundColor:passed?color+'15':'rgba(0,10,20,0.7)'}]}>
...
//         <Animated.View style={[hc.fill,{width:barW,backgroundColor:barColor,shadowColor:color,shadowOpacity:glowAnim,shadowRadius:8,elevation:4}]}/>
//       </View>
//     </View>
//   );
// });
...
//   chip: {width:85,borderWidth:1,borderRadius:2,padding:6,paddingBottom:5,marginBottom:8},
//   row:  {justifyContent:'space-between',marginBottom:4},
//   lbl:  {fontFamily:FONTS,fontSize:8,fontWeight:'800',letterSpacing:0.5},
//   val:  {fontFamily:FONTS,fontSize:10,fontWeight:'900'},
//   track:{height:3,backgroundColor:'rgba(255,255,255,0.05)'},
//   fill: {height:3},
//   tmark:{position:'absolute',top:-2,width:2,height:7,backgroundColor:'rgba(255,255,255,0.4)',zIndex:3},
// });
...
//   useEffect(()=>{
//     Animated.loop(Animated.timing(r1,{toValue:1,duration:4000,easing:Easing.linear,useNativeDriver:true})).start();
//     Animated.loop(Animated.timing(r2,{toValue:1,duration:6000,easing:Easing.linear,useNativeDriver:true})).start();
//   },[]);
//   useEffect(()=>{
//     if(allPass){
//       Animated.loop(Animated.sequence([
//         Animated.timing(pulse,{toValue:1.03,duration:800,useNativeDriver:true}),
//         Animated.timing(pulse,{toValue:0.97,duration:800,useNativeDriver:true}),
//       ])).start();
...
//   const cw=OVAL_W+16,ch=OVAL_H+16;
//   return (
//     <Animated.View style={[ri.wrap,{transform:[{scale:pulse}]}]} pointerEvents="none">
//       <View style={[ri.bracket,{top:-15,left:-15,borderTopWidth:3,borderLeftWidth:3,borderColor:gc}]}/>
//       <View style={[ri.bracket,{top:-15,right:-15,borderTopWidth:3,borderRightWidth:3,borderColor:gc}]}/>
//       <View style={[ri.bracket,{bottom:-15,left:-15,borderBottomWidth:3,borderLeftWidth:3,borderColor:gc}]}/>
//       <View style={[ri.bracket,{bottom:-15,right:-15,borderBottomWidth:3,borderRightWidth:3,borderColor:gc}]}/>

//       <Animated.View style={[ri.arc,{width:cw,height:ch,transform:[{rotate:spin1}]}]}>
//         <View style={{width:cw,height:ch,borderRadius:cw/2,borderWidth:2,borderColor:'transparent',borderTopColor:gc+'88',borderRightColor:gc+'33',position:'absolute'}}/>
//       </Animated.View>
//       <Animated.View style={[ri.arc,{width:cw+20,height:ch+20,transform:[{rotate:spin2}]}]}>
//         <View style={{width:cw+20,height:ch+20,borderRadius:(cw+20)/2,borderWidth:1,borderColor:'transparent',borderBottomColor:gc+'AA',borderLeftColor:gc+'44',borderStyle:'dashed',position:'absolute'}}/>
//       </Animated.View>

//       <View style={[ri.arc,{width:OVAL_W-16,height:OVAL_H-16}]}>
//         <View style={{width:OVAL_W-16,height:OVAL_H-16,borderRadius:(OVAL_W-16)/2,borderWidth:3,borderColor:'transparent',
//           borderTopColor:allPass?C.green:color, borderRightColor:progress>0.25?(allPass?C.green:color):'transparent',
//           borderBottomColor:progress>0.50?(allPass?C.green:color):'transparent', borderLeftColor:progress>0.75?(allPass?C.green:color):'transparent',
//           position:'absolute'}}/>
//       </View>
//     </Animated.View>
//   );
...
//       ]).start();
//       Animated.loop(Animated.timing(ro,{toValue:1,duration:4000,easing:Easing.linear,useNativeDriver:true})).start();
//     } else { Animated.timing(op,{toValue:0,duration:150,useNativeDriver:true}).start(); }
//   },[visible]);
...
//       <Animated.View style={{position:'absolute',width:S,height:S,borderRadius:S/2,borderWidth:1.5,borderColor:color+'88',transform:[{rotate}],borderStyle:'dashed'}}/>
//       <View style={{position:'absolute',width:S*0.3,height:S*0.3,borderWidth:1.5,borderColor:color}}/>
//       <View style={{position:'absolute',height:1,width:S,backgroundColor:color+'88'}}/>
//       <View style={{position:'absolute',width:1,height:S,backgroundColor:color+'88'}}/>
//       <View style={{position:'absolute',width:4,height:4,backgroundColor:color}}/>
//     </Animated.View>
//   );
// };
...
//   },[alert]);
//   return (
//     <Animated.View style={[tc.guideBox,{borderColor:color,opacity:anim,backgroundColor:alert?'rgba(255,0,60,0.2)':'rgba(0,10,20,0.85)'}]}>
...
//     op.setValue(0.85);
//     Animated.loop(Animated.timing(y,{toValue:OVAL_H,duration:2200,easing:Easing.linear,useNativeDriver:true})).start();
//   },[active]);
//   return (
...
//         <View style={{width:'100%',height:25,backgroundColor:'rgba(0,0,0,0)'}}/>
//         <View style={{width:'100%',height:2,backgroundColor:color+'CC'}}/>
//         <View style={{width:'100%',height:25,backgroundColor:'rgba(0,0,0,0)',transform:[{scaleY:-1}]}}/>
//       </Animated.View>
//     </Animated.View>
//   );
// });
...
//   useEffect(()=>{
//     if(visible){
//       sc.setValue(0.3);op.setValue(1);
//       Animated.parallel([
//         Animated.timing(sc,{toValue:3.8,duration:650,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
//         Animated.timing(op,{toValue:0,duration:650,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
//       ]).start();
//     }
//   },[visible]);
//   return <Animated.View style={{position:'absolute',width:OVAL_W,height:OVAL_H,borderRadius:OVAL_W/2,borderWidth:4,borderColor:color,transform:[{scale:sc}],opacity:op}} pointerEvents="none"/>;
// };
...
//     ])).start();
//   },[]);
//   return <Animated.View style={{width:10,height:10,borderRadius:5,backgroundColor:color,opacity:op,marginHorizontal:6}}/>;
// }
...
//       clr.current=c; op.setValue(0.65);
//       Animated.timing(op,{toValue:0,duration:600,useNativeDriver:true}).start();
//     };
//   },[]);
...
//       if(aligned){
//         e.baseEAR = Math.max(e.baseEAR, avgEAR); // Constantly find the widest open eye baseline
...
//         e.baseEAR = Math.max(e.baseEAR, currentEAR); // Update baseline dynamically if eyes open wider
        
//         // Massive robustness: A closed eye drops significantly compared to the baseline
...
//         // Use Absolute values so it's impossible to fail from a bad baseline
...
//     // Completely wipe all progress logic for the new challenge baseline
//     engRef.current={stage:0,alignFrames:0,baseEAR:0,baseIris:0.5,baseNoseY:0.5,baseYaw:0,basePitch:0,
...
//   if(phase==='EXPIRED')    return <ResultScreen icon="[TIME_EXC]" title="SYS_TIMEOUT"      sub="AUTH WINDOW CLOSED"             color={C.yellow} onHome/>;
//   if(phase==='SUSPICIOUS') return <ResultScreen icon="[WARN]"  title="SYS_REVIEW" sub="MANUAL CLEARANCE REQUIRED"            color={C.yellow} onHome/>;
//   if(phase==='FAILED')     return <ResultScreen icon="[ERR]"  title="SYS_FAILED"        sub={submitError||'VERIFICATION ABORTED'} color={C.red}    onRetry={retryAll} onHome/>;
...
//       <View style={[StyleSheet.absoluteFill, {paddingTop: OVAL_TOP, paddingHorizontal: 15, flexDirection: 'row', justifyContent: 'space-between'}]} pointerEvents="none">
...
//           <Text style={[tc.matrix, {textAlign:'right'}]}>SYS_OP_NORM</Text>
//           <Text style={[tc.matrix, {textAlign:'right', color:C.teal}]}>BIOM_SCAN_V9</Text>
//           <View style={{height:15}}/>
...
//         <View style={{position:'absolute', bottom:-100, width:SW-40, alignItems:'center'}}>
//            <GuidanceOverlay 
...
//             <Text style={tc.procSub}>MAINTAIN POSITION</Text>
//             <View style={{flexDirection:'row',marginTop:18}}>
//               {[0,1,2].map(i=><AnimDot key={i} delay={i*200} color={C.cyan}/>)}
//             </View>
//           </View>
//         </View>
//       )}
...
//               {phase==='CHALLENGE'?`SEQ ${completedList.length}/${challengeKeys.length}`:'INITIALIZING'}
//             </Text>
//           </View>
//           <View style={[tc.timerPill,urgent&&tc.timerUrgent]}>
//             <Text style={[tc.timerTxt,urgent&&{color:C.red}]}>{urgent?'WARN: ':''}{fmt(sessTimeLeft)}</Text>
//           </View>
//         </View>
...
//             <Text style={[tc.consoleHeader,{color:C.cyan}]}>{'// INITIALIZE BIOMETRIC HANDSHAKE'}</Text>
//             <Text style={tc.consoleBody}>{challengeKeys.length} TARGET SEQUENCES DETECTED.</Text>
//             <View style={tc.seqRow}>
//               {challengeKeys.map((k,i)=>{ const d=DEFS[k]||DEFS['BLINK_TWICE']; return (
//                 <View key={i} style={[tc.seqItem,{borderColor:d.color}]}>
//                   <Text style={{fontSize:16,color:d.color,marginBottom:6}}>{d.icon}</Text>
...
//               <Text style={[tc.consoleHeader,{color:chipColor}]}>
//                 {isLiveness ? '// LIVENESS_BUFFER' : `// SEQ_RUNNING: ${chState.idx+1}/${challengeKeys.length}`}
//               </Text>
//               <Text style={[tc.consoleHeader,{color:chipColor}]}>
//                 {isLiveness ? `${(livenessProgress*100).toFixed(0)}%` : `${(chState.progress*100).toFixed(0)}%`}
//               </Text>
//             </View>
//             <View style={tc.progressBar}>
//               <Animated.View style={[tc.progressFill, {backgroundColor:chipColor, width: `${(isLiveness?livenessProgress:chState.progress)*100}%`}]}/>
//             </View>
//             <Text style={tc.consoleBody}>
//               {'>'} {isLiveness?'AWAITING LIVENESS CONFIRMATION':curDef?.instruction}
//             </Text>
//             <Text style={[tc.consoleBody,{color:C.dimmer}]}>
//               {'>'} {isLiveness?'STAY IN FRAME':curDef?.hint}
//             </Text>
//             {phase==='CHALLENGE'&&(
//               <View style={[tc.statusRow, {borderColor:chState.status==='PASSED'?C.green:chState.timeLeft<=3?C.red:oColor}]}>
//                 <Text style={[tc.statusTxt,{color:chState.status==='PASSED'?C.green:chState.timeLeft<=3?C.red:oColor}]}>
//                   {chState.status==='PASSED'?'STATUS: VERIFIED':chState.status==='ALIGNING'?'STATUS: ALIGNING':chState.status==='FAILED_RETRY'?'STATUS: RESTARTING':`T-${chState.timeLeft}s`}
//                 </Text>
//               </View>
//             )}
//           </Animated.View>
...
//   },[]);
//   return (
//     <Animated.View style={{alignItems:'center',transform:[{scale}],opacity:op,padding:32}}>
//       <View style={sr.ring}><Text style={{fontSize:40,color:C.green,fontFamily:FONTS}}>[OK]</Text></View>
//       <Text style={[sr.title,{color:C.green,marginTop:28}]}>AUTH_SUCCESS</Text>
//       <Text style={sr.sub}>DATA LOGGED FOR:{'\n'}<Text style={{color:C.white,fontWeight:'800'}}>{courseName||'TARGET'}</Text></Text>
//       <View style={sr.meta}>
//         <Text style={sr.metaRow}>{'// SEQ_PASSED: '}{challenges}</Text>
//         <Text style={sr.metaRow}>{'// TIMESTAMP: '}{new Date().getTime()}</Text>
//       </View>
//       <TouchableOpacity style={sr.btn} onPress={()=>router.replace('/(tabs)/home')}>
//         <Text style={sr.btnTxt}>[ TERMINATE ]</Text>
//       </TouchableOpacity>
//     </Animated.View>
//   );
// }
...
//         <View style={[sr.termRing,{borderColor:color,shadowColor:color}]}><Text style={{fontSize:24,color,fontFamily:FONTS}}>{icon}</Text></View>
//         <Text style={[sr.title,{color,marginTop:22}]}>{title}</Text>
//         <Text style={sr.sub}>{sub}</Text>
//         {onRetry&&<TouchableOpacity style={[sr.btn,{borderColor:color,backgroundColor:color+'11'}]} onPress={onRetry}><Text style={[sr.btnTxt,{color}]}>[ RETRY_SEQ ]</Text></TouchableOpacity>}
//         {onHome&&<TouchableOpacity style={{marginTop:24}} onPress={()=>router.replace('/(tabs)/home')}><Text style={{color:C.dimmer,fontFamily:FONTS,fontSize:10}}>{'[ EXIT_TO_MAIN ]'}</Text></TouchableOpacity>}
//       </Animated.View>
//     </View>
//   );
// }
...
//   matrix:       {fontFamily:FONTS,fontSize:8,color:C.dimmer,lineHeight:12},
  
//   guideBox:     {borderWidth:1.5,paddingHorizontal:16,paddingVertical:12,borderLeftWidth:6,borderRightWidth:6,width:'100%',shadowOpacity:0.5,shadowRadius:10,elevation:5},
//   guideTxt:     {fontFamily:FONTS,fontSize:11,fontWeight:'bold',textAlign:'center',letterSpacing:1.5},

//   procOverlay:  {...StyleSheet.absoluteFillObject,backgroundColor:'rgba(0,5,10,0.9)',justifyContent:'center',alignItems:'center',zIndex:100},
//   procBox:      {borderWidth:1,borderColor:C.cyan+'44',padding:30,alignItems:'center',backgroundColor:'rgba(0,229,255,0.05)'},
//   procTitle:    {color:C.cyan,fontFamily:FONTS,fontSize:14,letterSpacing:2,marginTop:22},
//   procSub:      {color:C.dim,fontFamily:FONTS,fontSize:10,marginTop:8},
...
//   backBtn:      {padding:8, backgroundColor:'rgba(0,0,0,0.5)', borderWidth:1, borderColor:C.dimmer},
//   backTxt:      {color:C.dim,fontFamily:FONTS,fontSize:10, letterSpacing:1},
//   topTitle:     {color:C.white,fontFamily:FONTS,fontSize:12,letterSpacing:2,maxWidth:160,textAlign:'center'},
//   topSub:       {fontFamily:FONTS,fontSize:9,letterSpacing:1,marginTop:4},
//   timerPill:    {paddingHorizontal:10,paddingVertical:6,borderWidth:1,borderColor:C.cyan+'55', backgroundColor:'rgba(0,0,0,0.6)'},
//   timerUrgent:  {borderColor:C.red},
//   timerTxt:     {color:C.cyan,fontFamily:FONTS,fontSize:11},

//   bottomConsole:{backgroundColor:'rgba(0,10,20,0.92)',borderTopWidth:1,borderColor:C.cyan+'44',padding:20,paddingBottom:30},
//   consoleLine:  {height:2,width:40,backgroundColor:C.cyan+'88',marginBottom:16},
//   consoleHeader:{fontFamily:FONTS,fontSize:11,letterSpacing:1,marginBottom:12,fontWeight:'700'},
//   consoleBody:  {fontFamily:FONTS,fontSize:11,color:C.dim,marginBottom:6,lineHeight:16},
//   progressBar:  {height:4,backgroundColor:'rgba(255,255,255,0.1)',marginBottom:16},
//   progressFill: {height:4,backgroundColor:C.cyan,shadowColor:C.cyan,shadowRadius:8,shadowOpacity:1},

//   seqRow:       {flexDirection:'row',flexWrap:'wrap',gap:10,marginVertical:16},
//   seqItem:      {borderWidth:1,paddingHorizontal:10,paddingVertical:10,alignItems:'center',backgroundColor:'rgba(255,255,255,0.03)', minWidth: 90},
//   seqLabel:     {fontFamily:FONTS,fontSize:9,marginTop:4, fontWeight:'700'},
  
//   execBtn:      {borderWidth:1,borderColor:C.cyan,backgroundColor:C.cyan+'11',paddingVertical:16,alignItems:'center',marginTop:8},
//   execTxt:      {color:C.cyan,fontFamily:FONTS,fontSize:12,letterSpacing:2, fontWeight:'bold'},

//   statusRow:    {borderWidth:1,paddingVertical:10,alignItems:'center',marginTop:12,backgroundColor:'rgba(0,0,0,0.5)'},
//   statusTxt:    {fontFamily:FONTS,fontSize:11,letterSpacing:1, fontWeight:'bold'},
// });

// const sr = StyleSheet.create({
//   ring:    {width:100,height:100,borderWidth:2,borderColor:C.green,shadowColor:C.green,shadowRadius:20,shadowOpacity:0.5,justifyContent:'center',alignItems:'center'},
//   termRing:{width:100,height:100,borderWidth:2,justifyContent:'center',alignItems:'center',shadowRadius:20,shadowOpacity:0.5},
//   title:   {fontFamily:FONTS,fontSize:18,letterSpacing:3,marginBottom:12,textAlign:'center'},
//   sub:     {fontFamily:FONTS,color:C.dim,fontSize:11,textAlign:'center',lineHeight:18,marginBottom:24},
//   meta:    {marginTop:16,borderWidth:1,borderColor:C.green+'44',paddingHorizontal:24,paddingVertical:16,gap:8,alignItems:'flex-start',backgroundColor:C.green+'0A',width:'100%'},
//   metaRow: {fontFamily:FONTS,color:C.dim,fontSize:10},
//   btn:     {borderWidth:1,borderColor:C.green,backgroundColor:C.green+'11',paddingVertical:16,width:'100%',alignItems:'center',marginTop:24},
//   btnTxt:  {fontFamily:FONTS,fontSize:12,letterSpacing:2,color:C.green, fontWeight:'bold'},
// });
  ...
  return (
    <View style={tc.fill}>
      ...
      <View style={[tc.ovalZone,{top:OVAL_TOP,left:(SW-OVAL_W)/2}]} pointerEvents="none">
        ...
        <View style={{position:'absolute', bottom:-110, width:SW-32, alignItems:'center'}}>
          <GuidanceOverlay
            feedback={livenessBlocked ? '[ CRITICAL: STATIC ENTITY ]' : chState.feedback}
            color={livenessBlocked ? C.red : (faceDetected ? (allPass ? C.green : chipColor) : C.orange)}
            alert={livenessBlocked || (!faceDetected && active) || chState.feedback.includes('OFFSET') || chState.feedback.includes('ERR')}
      ...
      <SafeAreaView style={tc.safe} pointerEvents={['CAPTURING','SUBMITTING'].includes(phase)?'none':'box-none'}>
        ...
        {(phase==='LIVENESS_CHECK'||phase==='CHALLENGE')&&(
          <Animated.View style={[tc.bottomConsole,{opacity:drawerFade,transform:[{translateY:drawerSlide}]}]}>
            ...
            {phase==='CHALLENGE'&&(
              <View style={[tc.statusRow,{borderColor:
                chState.status==='PASSED'?C.green:chState.timeLeft<=3?C.red:oColor
              }]}>
                <View style={[tc.statusAccent,{backgroundColor:
                  chState.status==='PASSED'?C.green:chState.timeLeft<=3?C.red:oColor
                }]}/>
                <Text style={[tc.statusTxt,{color:
                  chState.status==='PASSED'?C.green:chState.timeLeft<=3?C.red:oColor
                }]}>
