...
export default function DeviceResetsPage() {
  ...
  async function loadRequests() {
    setLoading(true);
    try {
      const res = await AdminAPI.listDeviceResets(statusFilter);
  ...
  async function handleApprove(id: string) {
    if (!confirm('Approve this device reset? The old device will be deactivated.')) return;
    setProcessing(id);
    try { await AdminAPI.approveReset(id); await loadRequests(); }
  ...
  async function handleReject() {
    ...
    try {
      await AdminAPI.rejectReset(rejectModal, rejectNote.trim());
  ...
  requests.forEach(r => { if (counts[r.status as keyof typeof counts] !== undefined) counts[r.status as keyof typeof counts]++; });
      ...
      <div className="sa-inner">
        ...
        <div className="sa-header">
          <div>
            ...
            <div><span className="sa-title">Device Resets</span><span className="sa-chip">{requests.length}</span></div>
            <p className="sa-subtitle">Review and approve student device binding resets</p>
          </div>
        </div>
        ...
        <div className="sa-card">
          <div className="sa-card-header"><div><p className="sa-card-title">Reset Requests</p><p className="sa-card-sub">{requests.length} {statusFilter.toLowerCase()} request{requests.length!==1?'s':''}</p></div></div>
          {loading ? (
                    ...
                    <td>
                      {req.status === 'PENDING' ? (
                        <div style={{display:'flex',gap:6}}>
                          <button className="btn-success" disabled={processing===req.request_id} onClick={()=>handleApprove(req.request_id)}>
                            {processing===req.request_id?'…':'✓ Approve'}
                          </button>
                          <button className="btn-danger" disabled={processing===req.request_id} onClick={()=>{setRejectModal(req.request_id);setRejectNote('');}}>
                            ✕ Reject
                          </button>
                        </div>
                      ) : (
          ...
          <div className="sa-modal-overlay" onClick={e=>e.target===e.currentTarget&&setRejectModal(null)}>
            <div className="sa-modal-box">
              ...
              <div className="sa-form-group"><label className="sa-form-label">Rejection Reason *</label>
                <textarea className="sa-form-textarea" rows={4} value={rejectNote} onChange={e=>setRejectNote(e.target.value)} placeholder="e.g. Insufficient proof provided. Please visit department office."/>
              </div>
