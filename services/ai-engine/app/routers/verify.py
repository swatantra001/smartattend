...
#     Step 2 — Scene verification: compare background to classroom baseline
#     """
#     logger.info(
...
#         logger.warning(
#             "No face detected in verification frame",
#             student_id=request.student_id,
#             det_score=det_score,
#         )
#         raise HTTPException(
...
#         # Only update baseline if face passed (verified students build baseline)
#         if face_passed:
#             sample_count = await scene_service.update_baseline(
#                 request.session_id, scene_vector
#             )
#             logger.info(
#                 "Scene baseline updated",
#                 session_id=request.session_id,
#                 sample_count=sample_count,
#             )

#         scene_score, is_suspicious = await scene_service.compute_scene_score(
#             request.session_id, scene_vector
#         )
#         scene_passed = not is_suspicious

#     except Exception as e:
#         # Scene verification failure must NOT block attendance
#         # Log and give full score (fail open for scene)
#         logger.error("Scene verification error", error=str(e))
#         scene_score = 1.0
#         scene_passed = True
...
#             )
#     except Exception as e:
#         logger.warning("Audit frame upload failed", error=str(e))
