...
//       throw new AppError(409, 'Roll number already registered', 'ROLL_EXISTS');
//     }
...
//       [body.college_id, body.email, passwordHash]
//     );
//     const userId = userResult.rows[0].user_id;

//     // Create student profile
//     await client.query(
//       `INSERT INTO students (user_id, dept_id, name, roll_number, semester)
//        VALUES ($1, $2, $3, $4, $5)`,
//       [userId, body.dept_id, body.name, body.roll_number, body.semester]
//     );
//   });
...
//       throw new AppError(409, 'Employee code already registered', 'CODE_EXISTS');
//     }
...
//     await client.query(
//       `INSERT INTO professors (user_id, dept_id, name, employee_code)
//        VALUES ($1, $2, $3, $4)`,
//       [userId, body.dept_id, body.name, body.employee_code]
//     );
//   });

//   res.status(201).json({
//     success: true,
//     message: 'Professor registered successfully. Please login.'
//   });
// }
...
// D:\smartattend\services\api\src\controllers\auth.controller.ts
// FULL REPLACEMENT — includes OTP forgot-password + admin-must-register-first guard
...
//   // Look up user by email, roll_number, or employee_code
//   let userRow: any = null;
//   let recipientEmail = '';
//   let recipientName = '';

//   // Try email first
//   userRow = await db.queryOne<any>(
//     `SELECT u.user_id, u.email, u.role, u.is_active
//      FROM users u WHERE LOWER(u.email) = LOWER($1)`,
//     [identifier]
//   );

//   logger.info(`User lookup for forgot password (email): ${userRow ? 'FOUND' : 'NOT FOUND'}`);

//   if (!userRow) {
//     // Try roll_number (student)
//     const student = await db.queryOne<any>(
//       `SELECT u.user_id, u.email, u.role, u.is_active, s.name
...
//   logger.info(`User lookup for forgot password (roll_number): ${userRow ? 'FOUND' : 'NOT FOUND'}`);

//   if (!userRow) {
//     // Try employee_code (professor)
//     const professor = await db.queryOne<any>(
//       `SELECT u.user_id, u.email, u.role, u.is_active, p.name
...
//   // ⚠ IMPORTANT: If user not found, we still respond generically to prevent enumeration
//   if (!userRow) {
//     res.json({
//       success: true,
//       message: 'If this identifier is registered, you will receive an OTP.'
//     });
//     return;
//   }
...
//   const passwordCheck = validatePasswordStrength(body.new_password);
//   if (!passwordCheck.valid) {
//     throw new AppError(400, passwordCheck.message!, 'WEAK_PASSWORD');
//   }
...
//   if (!otpRecord) {
//     throw new AppError(400, 'No valid OTP found. Please request a new one.', 'OTP_NOT_FOUND');
//   }
...
//   const otpMatch = await bcrypt.compare(body.otp, otpRecord.otp_hash);
//   if (!otpMatch) {
...
//   // Mark OTP as used
//   await db.query(
//     'UPDATE otp_verifications SET used = TRUE WHERE otp_id = $1',
//     [otpRecord.otp_id]
//   );
...
//   } else {
//     const byRoll = await db.queryOne<{ user_id: string }>(
//       'SELECT user_id FROM students WHERE roll_number = $1',
//       [body.identifier]
//     );
//     if (byRoll) {
//       userId = byRoll.user_id;
//     } else {
//       const byCode = await db.queryOne<{ user_id: string }>(
//         `SELECT u.user_id FROM professors p JOIN users u ON u.user_id = p.user_id
//          WHERE p.employee_code = $1`,
//         [body.identifier]
//       );
//       if (byCode) userId = byCode.user_id;
//     }
//   }
