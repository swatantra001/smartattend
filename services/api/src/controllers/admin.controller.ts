...
//   // Check duplicate email in pre-registered pool
...
// // ─── BULK IMPORT STUDENTS (CSV/Excel/JSON) ────────────────────────────────────
// // POST /api/admin/students/bulk-import
// // Body: { students: [...] }
// // Frontend parses CSV/Excel and sends normalized JSON
...
//   // Collect all roll numbers in batch to check duplicates efficiently
...
//   // Validate dept IDs belong to college
...
// // POST /api/admin/professors/pre-register
...
// // POST /api/admin/professors/bulk-import
...
//   // Verify dept belongs to college
...
//   // Validate all student_ids exist in this college
...
// // POST /api/admin/courses/:courseId/enroll-by-rolls
// // This is used when admin adds a course and selects students — backend returns
// // which roll_numbers were not found / not registered yet
...
//   // Fetch students matching the roll numbers in this college
...
//     // i think it should be better if i delete the row instead of marking it inactive, because if we mark it inactive then the user can reactivate it by logging in from that device, but if we delete the row then the user cannot reactivate it, and they have to contact admin to reactivate it, which is more secure
//     await client.query(
...
//     // await client.query(
//     //   'UPDATE device_bindings SET is_active = FALSE WHERE user_id = $1',
//     //   [request.user_id]
//     // );
...
//          resolved_at = NOW()
//      WHERE request_id = $1`,
//     [requestId, req.user!.user_id, admin_note]
//   );
...
//     conditions.push(`asess.course_id = $${params.length}`);
//   }
//   if (from_date) {
//     params.push(from_date);
//     conditions.push(`asess.started_at >= $${params.length}`);
//   }
//   if (to_date) {
//     params.push(to_date);
//     conditions.push(`asess.started_at <= $${params.length}`);
...
//   // FIX: Join the students table to get the user_id!
//   const request = await db.queryOne<any>(
...
// D:\smartattend\services\api\src\controllers\admin.controller.ts
